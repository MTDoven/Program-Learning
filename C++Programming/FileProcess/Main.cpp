...
reload:
		...
		cout << "\nEnter new file name: ";
		string secend_file;  cin >> secend_file; 
		getchar(); // clear
		ifstream fileptr2 = ifstream(secend_file);
		if (!fileptr2.is_open()) {
			cout << "Input a proper file name (path to a file).\n";
			pause(); goto start;
		} // Ensure file exists and is readable.
		string str2 = file_to_string(fileptr2);
		string different_string = compare_strings(str, str2);
		if (different_string.empty()) {
			cout << "No different between these two files" << endl;
		} else { // different
			cout << "Difference in the these two lines below: " << endl;
			cout << different_string;
		} // show
		pause(); goto start;
	}
